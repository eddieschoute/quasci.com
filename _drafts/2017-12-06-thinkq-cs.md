...
One side of the discussion is determining quantum processes can be efficiently simulated
...
For any quantum circuit $\mathcal C$ there exists some probability distribution $\mathcal P$ over the outcomes $X=(X_1, X_2, …, X_k)$,
which is just a classical random variable.
Any noiseless circuit $\mathcal C$ can be described as starting with $ρ_1, … ρ_n$ product states,
...
output a sample in accordance with the output distribution $\mathcal P$.
...
and thus cannot even sample exactly from $\mathcal P$.
...
$L_1$ norm of discrete probability distributions
: For a probability distribution $\mathcal P$ the $L_1$ norm is defined as
  ...
  which is just taking the vector difference of the two probability distributions.
