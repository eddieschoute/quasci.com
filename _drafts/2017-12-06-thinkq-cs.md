...
1. Small ε: if $ε < 1/2^n$ then explicitly compute the quantum probability $p=P(X=1)$.
...
The problem here is that we have thinned the probability of any one string occurring so thinly,
that for a sufficiently low error ε it becomes easier to compute quantum probability explicitly.
If we have another condition which assures us that there are only a few relevant outputs,
i.e. the outputs are _sparse_,
then if there exists a poly-box we can ε-simulate.
The _poly-sparsity_ of discrete probability distributions and families is sufficient,
that is (roughly),
there must be some polynomial upper bound in $ε^{-1}$ on the number of relevant outputs.
